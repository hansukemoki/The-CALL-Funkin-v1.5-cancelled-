...
function bullInit(i)
    ...
    if (t == 'blaster') then
        ...
        bProp[n..'.x'] = bProp[n..'.x'] - math.cos(math.rad(bProp[n..'.iniAng'])) * 2180;
        bProp[n..'.y'] = bProp[n..'.y'] + math.sin(math.rad(bProp[n..'.iniAng'])) * 2180;
    ...
    elseif (t == 'blast') then
        ...
        setProperty(n..'.scale.x', 8);
        setProperty(n..'.scale.y', 7);
        setProperty(n..'.alpha', 1);
    ...
    for i = 0, bInd - 1, 1 do
        ...
        if ((bProp[n..'.unspawn'] == false and bProp[n..'.safe'] == false and canMove) and hitIt) then
            if (hRemove == false) then
                ...
                setProperty('health', getProperty('health') - (elapsed / 0.3) * diffDmg * indDmg * healthLossMult);
    ...
    if (tp == 'blaster') then
        ...
        elseif (s == 1) then
            ...
            if (t == 48) then
                ...
                triggerEvent('Screen Shake', '0.2,0.010', '0.2,0.025');
            end
        elseif (s == 2) then
        ...
        setProperty(n..'.scale.y',0.9 * bProp[n..'.extend'] * bProp[n..'.exArg']);
        setProperty(n..'.scale.x',0.8 * (1 + (1 - bProp[n..'.extend']) / 3));
    elseif (tp == 'blast') then
        bProp[n..'.h'] = 16 * getProperty(n..'.scale.y');
        bProp[n..'.sc'] = bProp[n..'.sc'] + (bProp[n..'.exDir'] * 1.7);
        ...
        setProperty(n..'.scale.y', bProp[n..'.sc'] * bProp[n..'.exArg'] * 2.2);
        bProp[n..'.safe'] = false;
...
function onCreate()
    ...
    setProperty('box.offset.x', 300);
    setProperty('box.offset.y', 300);
    setProperty('box.antialiasing', false);
    setObjectCamera('box', 'camGAME');
    scaleObject('box', 1, 1);

    makeLuaSprite('boxB', 'ut/boxBorder', boxX, boxY);
    setProperty('boxB.offset.x', 300);
    setProperty('boxB.offset.y', 300);
    setProperty('boxB.antialiasing', false);
    setObjectCamera('boxB', 'camGAME');
    scaleObject('boxB', 1, 1);

    addLuaSprite('boxB', false);
    addLuaSprite('box', true);

    makeLuaSprite('p', 'ut/soul', boxX, boxY);
    setProperty('p.offset.x', 9);
    setProperty('p.offset.y', 9);
    setProperty('p.scale.x', 2.5);
    setProperty('p.scale.y', 2.5);
...
--ESPACIO
    local lBd = -boxW / 1.04 + 16;
    local rBd = boxW / 0.72 - 16;
    local uBd = -boxH / 0.99 + 16;
    local dBd = boxH / 1.38 - 16;
    ...
    if (ground and heavy) then
        triggerEvent('Screen Shake', '0.2,0.35', '0.2,0.1');
        playSound('impact', 1);
        act = 1;
        heavy = false;
    end
...
function atkStep()
    ...
    elseif (k == 'platformRush') then
        ...
        if (atkTimer == 480) then
            bullSpawn('blaster', -200, -300, 0, 0, 1, 270, 1.);
        end
