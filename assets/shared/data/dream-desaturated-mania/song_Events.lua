...
function onCreate()
	...
	makeAnimatedLuaSprite("intro","stages_TMK/gameboy",0,0)
	setObjectCamera('intro', 'camOther')
	addAnimationByPrefix('intro', 'idle', 'idle', 24, false)
	setGraphicSize('intro',screenWidth,screenHeight)
	screenCenter('intro')
  ...
  if curStep == 2384 then
	...
	if curStep == 1072 then
		...
		if not middlescroll then
			...
			noteTweenAlpha("o1", 0, 0.35, 0.6);
			noteTweenAlpha("o2", 1, 0.35, 0.6);
			noteTweenAlpha("o3", 2, 0.35, 0.6);
			noteTweenAlpha("o4", 3, 0.35, 0.6);
		end
