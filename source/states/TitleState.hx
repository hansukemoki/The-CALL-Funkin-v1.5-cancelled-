...
function showIntroBG(bgName:String)
{
    // Tạo hình nền mới
    introBG = new FlxSprite().loadGraphic(Paths.image(bgName));
    introBG.scrollFactor.set();
    introBG.screenCenter();
    introBG.alpha = 0; // bắt đầu mờ ẩn
    insert(0, introBG);
    add(introBG);

    // Làm mờ dần ảnh nền (fade in)
    FlxTween.tween(introBG, {alpha: 0.6}, 1, {ease: FlxEase.quadOut});
	FlxTween.tween(textGroup, {alpha: 0}, 0.8, {ease: FlxEase.quadInOut});
}
			...
			switch (sickBeats)
{
    ...
    case 2:
        showIntroBG('night2'); // <-- ảnh 1
        createCoolText(['a mod'], 40);

    case 4:
        addMoreText('made by', 40);
        addMoreText('vietnamese student', 40);

    case 5:
        deleteCoolText();

    case 6:
        showIntroBG('THPT'); // <-- ảnh 3
        createCoolText(['we are', 'studying on'], -40);

    case 8:
        addMoreText('Dinh Hoa Senior High School', -40);
        ngSpr.visible = true;
				case 9:
					deleteCoolText();
					ngSpr.visible = false;
